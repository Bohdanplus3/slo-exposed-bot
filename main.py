...
WRITER_SYSTEM = """Ti si novinar SLO.EXPOSED. Pišeš kratke, udarne objave za Telegram.

PRAVILA:
- 3-5 stavkov maksimum
- Piši kot človek, ne robot
- Samo fakti: ime, znesek, datum, funkcija
- Nikoli: "po poročanju", "kot kaže", "viri navajajo"
- Na koncu zadnjega stavka OBVEZNO en emoji — izberi glede na temo:
  💰 kraja, denar, korupcija, podkupovanje
  🔒 zapor, umor, obsodba, aretacija
  📁 preiskava, dokumenti, tožilstvo
  🏛️ politika, vlada, parlament, volitve
  🏗️ gradnja, infrastruktura, javna naročila
  🏳️ poraz, absurd, kapitulacija
  ☢️ okoljska katastrofa, nevarnost
  🎯 ironija, zadetek
  🧊 zamrznitev, blokada
- Piši SAMO v slovenščini
- Zadnja vrstica (nova vrstica): @slo_exposed

PRIMER:
Janez Hribar, direktor državnega podjetja STO d.o.o., je med 2021 in 2023 nakazal 840.000 € na račune treh podjetij svojega brata. KPK primer preučuje že leto dni. Hribar ostaja na funkciji. 💰
@slo_exposed"""
...
async def write_post(news: dict, editor_feedback: str = "") -> str:
    ...
    feedback_block = ""
    if editor_feedback:
        feedback_block = f"\n\nPREJŠNJI POST JE BIL ZAVRNJEN. Razlog: {editor_feedback}\nPopravi te napake."
...
1. Stil: suh, direkten, brez vode?
2. Dolžina: 3-5 stavkov?
3. Fakti: konkretni (ime, znesek, datum)?
4. Emoji: en emoji na koncu zadnjega stavka?
5. Konec: zadnja vrstica je @slo_exposed?
6. Jezik: slovenščina?
...
async def publish_post(bot: Bot, post: str, source_url: str) -> bool:
    ...
    try:
        ...
        log.info(f"[PUBLISHER] ✅ Опубликовано {'с фото' if image_url else 'без фото'} (#{state['stats']['published']} сегодня)")
...
async def cmd_resume(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    queued = state["queue"][:]
    state["queue"].clear()
    await update.message.reply_text(f"▶️ Публикации возобновлены. Публикую {len(queued)} постов из очереди...")
    for item in queued:
